...
export default function CoursePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (chapterParam) {
        ...
        setCurrentChapter(enrollment.current_chapter || 1)
      }
  ...
  async function markComplete() {
    ...
    const allDone = Object.keys(newProgress).filter(k => newProgress[k]).length >= 10
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#f8fafc' }}>
      ...
      <nav style={{ backgroundColor: '#0f2040', padding: '14px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 10 }}>
        <Link href="/dashboard" style={{ fontFamily: 'Sora, sans-serif', color: '#f59e0b', fontWeight: 700, fontSize: '1rem', textDecoration: 'none' }}>
          ← Dashboard
        </Link>
        ...
        <main style={{ flex: 1, padding: '32px 0 32px 24px', minWidth: 0 }}>
          <div style={{ backgroundColor: '#ffffff', borderRadius: '16px', padding: '40px', border: '1px solid #e2e8f0' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
              <span style={{ backgroundColor: '#f59e0b', color: '#0f2040', fontWeight: 700, fontSize: '0.75rem', padding: '3px 10px', borderRadius: '20px' }}>
            ...
            <h1 style={{ fontFamily: 'Sora, sans-serif', color: '#0f2040', fontSize: '1.75rem', fontWeight: 700, marginBottom: '32px' }}>
              {chapter.title}
            </h1>

            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', marginBottom: '40px' }}>
              {chapter.content.map((paragraph, i) => (
                <p key={i} style={{ color: '#374151', lineHeight: 1.8, fontSize: '1rem', margin: 0 }}>
                  {paragraph}
                </p>
              ))}
            </div>
            ...
            {/* Navigation */}
            <div style={{ borderTop: '1px solid #e2e8f0', paddingTop: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' as const, gap: '12px' }}>
              <button
                onClick={() => { if (currentChapter > 1) { setCurrentChapter(currentChapter - 1); window.scrollTo({ top: 0 }) } }}
                disabled={currentChapter === 1}
                style={{ color: currentChapter === 1 ? '#cbd5e1' : '#1e3a6e', background: 'none', border: 'none', cursor: currentChapter === 1 ? 'not-allowed' : 'pointer', fontWeight: 600, fontSize: '0.9rem' }}
              >
                ← Previous
              </button>
              ...
              {!isComplete ? (
                ...
                <button
                  onClick={() => { if (currentChapter < 10) { setCurrentChapter(currentChapter + 1); window.scrollTo({ top: 0 }) } else { router.push('/exam') } }}
